...
setTimeout(function() {
	...
	urls[1] = 'https://api.map.baidu.com/telematics/v3/weather?output=json&ak=0KZtF6IPrdr0mOlx4Kwn3nQj7IrxqKuF&callback=getTodayWeather&location=' + encodeURI(cityName);
...
function getWeather_week(response) {
	...
	for (var index in result) {
		...
		mess[2].innerHTML = result[index].wind;
		mess[3].innerHTML = result[index].winp;
		var span = mess[4].getElementsByTagName('span');
...
function getTodayWeather(response) {
	...
	var today = response.results[0].weather_data[0];
	var a = 0;
	for (var index in result) {
		for (var item in result[index]) {
			if (item == 'des') {
				p[a].innerHTML = result[index][item];
				a++;
			}
		}
	}
    var weathertips = doc.querySelector('.warmtips');
    var span1 = weathertips.getElementsByTagName('span');
    span1[1].innerHTML = result[0].zs;
    span1[3].innerHTML = result[1].zs;
    span1[5].innerHTML = result[2].zs;
    span1[7].innerHTML = result[3].zs;
    span1[9].innerHTML = result[4].zs;
	...
	//span[4].innerHTML = response.results[0].pm25;
}
